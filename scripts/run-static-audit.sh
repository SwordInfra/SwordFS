...
if ! command -v run-clang-tidy >/dev/null 2>&1; then
  ...
  TIDY_HEADER_REGEX=$(
    "$PYTHON_BIN" -c 'import re, sys; print("^" + re.escape(sys.argv[1]) + r"/src/.*")' "$ROOT_DIR"
  )
  TIDY_FILE_REGEX=$(
    "$PYTHON_BIN" -c 'import re, sys; print("^" + re.escape(sys.argv[1]) + r"/src/.*\.(?:cc|cpp|cxx)$")' "$ROOT_DIR"
  )
  run-clang-tidy \
    -p "$BUILD_DIR" \
    -j "$(nproc)" \
    ...
    -warnings-as-errors='*' \
    -header-filter="$TIDY_HEADER_REGEX" \
    "$TIDY_FILE_REGEX" || tidy_status=$?
fi
