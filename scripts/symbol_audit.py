...
def _eligible_declaration(cursor: Any, cindex: Any, repo_root: Path) -> bool:
    ...
    if cursor.kind == cindex.CursorKind.FUNCTION_DECL:
        return parent.kind in namespace_parents and cursor.spelling != "main"
...
def _parse_repository(repo_root: Path, build_dir: Path) -> tuple[dict[str, Symbol], int]:
    cindex = _load_clang()
...
def analyze(
    ...
    symbols, translation_unit_count = _parse_repository(repo_root, build_dir)
    findings = [
        finding
...
def main() -> int:
    args = _parse_args()
    try:
        report = analyze(args.repo_root, args.build_dir, args.suppressions)
