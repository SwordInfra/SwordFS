...
`kUnwatched` read mode exists for validation-only reads whose same-key
concurrent changes are explicitly outside the transaction's correctness
snapshot; reclaim uses it for per-inode fields inside shared lifecycle Hashes
and for the globally changing inode counter's type validation. All reads must
precede the first queued write; the wrapper rejects reads after writes.
...
2. **Finalize** validates the frozen record and orphan marker, then starts from
   a fresh watched snapshot of the still-unlinked live inode and
   public/private chunk state. The frozen/orphan fields live in shared Hashes,
   so their per-inode values are validated without WATCHing the whole shared
   key: after freeze they are immutable for that inode until finalization and
   completion, while unrelated inode fields may continue to change.
   `inode_count` is likewise validated as a positive signed Redis integer
   without WATCHing its numeric value. Legitimate concurrent `INCRBY`
   operations preserve that type invariant and are not part of this inode's
   reclaim snapshot. The transaction then applies strategy-private reclaim
   changes, removes public chunk heads and the orphan marker, decrements
   `inode_count`, and deletes the live inode.
...
This WATCH scope is deliberately inode-local. Redis WATCH tracks whole keys,
not individual Hash fields. Watching the shared `reclaims` or `orphans`
Hashes, or the globally changing `inode_count` key, would make independent
inode lifecycle operations conflict and can turn reclaim into a global
serialization point. Schema corruption racing finalization is outside the
normal protocol; supported writers preserve the validated Redis types.
