...
utils::Status Chunk::Flush() {
  ...
  if (refresh_publication_baseline_) {
    auto status = RefreshPublicationBaseline();
    if (!status.ok()) {
      return fail(status);
    }
  }
  ...
  const auto replacement = BuildMeta(revision);
  ...
  status = meta_->CommitChunk(ino_, published_chunk_, replacement);
  if (!status.ok()) {
  ...
  CompletePublication(replacement);
...
utils::Status Chunk::RefreshPublicationBaseline() {
