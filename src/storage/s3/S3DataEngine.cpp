...
bool S3DataEngine::Head(std::string_view key, size_t* size) {
  ...
  auto outcome = slot.client->HeadObject(req);
  if (!outcome.IsSuccess()) return false;
  if (size) *size = static_cast<size_t>(outcome.GetResult().GetContentLength());
  return true;
}

S3DataEngine::S3DataEngine(const S3Config& config, size_t pool_size)
    : cfg_(config), slots_(std::max(pool_size, size_t(1))) {
  for (auto& slot : slots_) {
    Aws::Client::ClientConfiguration aws_cfg;
    aws_cfg.endpointOverride = cfg_.endpoint;
    aws_cfg.region = cfg_.region;
    slot.client = std::make_unique<Aws::S3::S3Client>(aws_cfg);
  }
  SWORDFS_LOG_INFO << "S3DataEngine: endpoint=" << cfg_.endpoint
                   << " bucket=" << cfg_.bucket
                   << " pool_size=" << slots_.size();
}

S3DataEngine::Slot& S3DataEngine::NextSlot() const {
  return slots_[next_slot_.fetch_add(1, std::memory_order_relaxed) %
                slots_.size()];
}
...
Status S3DataEngine::Put(std::string_view key, std::string_view data) {
  ...
  auto& slot = NextSlot();
  std::lock_guard<std::mutex> lock(slot.mu);
  auto outcome = slot.client->PutObject(req);
...
Status S3DataEngine::Get(std::string_view key, std::string* out,
  ...
  auto& slot = NextSlot();
  std::lock_guard<std::mutex> lock(slot.mu);
  auto outcome = slot.client->GetObject(req);
  if (!outcome.IsSuccess()) {
...
Status S3DataEngine::Delete(std::string_view key) {
  ...
  auto& slot = NextSlot();
  std::lock_guard<std::mutex> lock(slot.mu);
  auto outcome = slot.client->DeleteObject(req);
  if (!outcome.IsSuccess()) {
