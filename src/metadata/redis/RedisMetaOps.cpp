...
utils::Status RedisMetaOps::LookupEntry(InodeID parent_ino, std::string_view name, SwordFsInode *out) {
  ...
  SwordFsInode result;
  auto status = Transact([&](RedisMetaTxn &txn) { return txn.LookupEntry(parent_ino, name, &result); });
  if (status.ok()) {
    *out = result;
  }
  return status;
...
utils::Status RedisMetaOps::Truncate(InodeID ino, uint64_t size) {
  return Transact([&](RedisMetaTxn &txn) { return txn.Truncate(ino, size); });
}

utils::Status RedisMetaOps::TouchInode(InodeID ino, SetAttrField fields) {
  return Transact([&](RedisMetaTxn &txn) { return txn.TouchInode(ino, fields); });
}

utils::Status RedisMetaOps::ReclaimInode(InodeID ino) {
  return Transact([&](RedisMetaTxn &txn) { return txn.ReclaimInode(ino); });
}

utils::Status RedisMetaOps::AddChunk(InodeID ino, const SwordFsChunk &chunk) {
  return Transact([&](RedisMetaTxn &txn) { return txn.AddChunk(ino, chunk); });
}
...
utils::Status RedisMetaOps::Transact(const std::function<utils::Status(RedisMetaTxn &)> &callback) {
