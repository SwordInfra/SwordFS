...
class MetaStore {
 public:
  ...
  virtual Status Lookup(InodeID parent, std::string_view name,
  ...
  virtual Status Create(InodeID parent, std::string_view name, mode_t mode,
                        InodeID* child_ino, struct stat* attr) = 0;

  /// Create a directory. Increments parent nlink to account for "..".
  virtual Status MkDir(InodeID parent, std::string_view name, mode_t mode,
                       InodeID* child_ino, struct stat* attr) = 0;

  /// Remove a regular file.
  virtual Status Unlink(InodeID parent, std::string_view name) = 0;

  /// Remove an empty directory. Decrements parent nlink.
  virtual Status RmDir(InodeID parent, std::string_view name) = 0;

  /// Rename (move) an entry between directories.
  virtual Status Rename(InodeID old_parent, std::string_view old_name,
                        InodeID new_parent,
                        std::string_view new_name) = 0;

  /// Set attributes for an inode.
  virtual Status SetAttr(InodeID ino, const struct stat* attr, int to_set,
