...
class MemMetaImpl : public IMetaEngine {
 public:
  ...
  // Temporary source-compatibility helpers for direct MemMetaImpl tests.
  Status Lookup(InodeID parent_ino, std::string_view name, InodeID *ino,
                struct stat *attr) {
    SwordFsInode out;
    Status status = Lookup(parent_ino, name, &out);
    if (status.ok()) {
      if (ino) *ino = out.ino;
  ...
  Status GetAttr(InodeID ino, struct stat *attr) {
    SwordFsInode out;
    Status status = GetInode(ino, attr ? &out : nullptr);
  ...
  Status Create(InodeID parent_ino, std::string_view name, mode_t mode,
    ...
    SwordFsInode out;
    Status status = Create(parent_ino, name, mode, (ino || attr) ? &out : nullptr);
    if (status.ok()) {
      if (ino) *ino = out.ino;
      if (attr) *attr = out.attr;
    }
  ...
  Status MkDir(InodeID parent_ino, std::string_view name, mode_t mode,
               InodeID *ino, struct stat *attr) {
    SwordFsInode out;
    Status status = MkDir(parent_ino, name, mode, (ino || attr) ? &out : nullptr);
    if (status.ok()) {
      if (ino) *ino = out.ino;
      if (attr) *attr = out.attr;
    }
