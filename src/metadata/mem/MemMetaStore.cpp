...
// ────────────────────────────────────────────────────────────────
// MemMetaTxn — transaction primitives.  Every method runs with the
// store lock held; the transaction is the Transact() callback's
// lifetime.  Reads return snapshot copies; writes are by-ino.
// ────────────────────────────────────────────────────────────────

Status MemMetaTxn::LookupInode(InodeID ino, SwordFsInode *out) {
  SwordFsInode *inode = store_->FindInodeLocked(ino);
  if (!inode) {
...
Status MemMetaTxn::LookupEntry(InodeID parent_ino, std::string_view name,
                               SwordFsInode *out) {
  SwordFsInode *parent = store_->FindInodeLocked(parent_ino);
  if (!parent) {
  ...
  SwordFsInode *inode = store_->FindEntryLocked(parent_ino, name);
  if (!inode) {
...
Status MemMetaTxn::AddEntry(InodeID parent_ino, std::string_view name,
  ...
  if (store_->FindEntryLocked(parent_ino, name) != nullptr) {
  ...
  store_->InsertInodeLocked(child);
  store_->LinkEntryLocked(parent_ino, name, child);
...
Status MemMetaTxn::MoveEntry(InodeID old_parent_ino,
  ...
  SwordFsInode *old_parent = store_->FindInodeLocked(old_parent_ino);
  if (!old_parent) {
  ...
  if (store_->FindEntryLocked(old_parent_ino, old_name) == nullptr) {
    return Status::NotFound("source entry not found");
  }

  SwordFsInode *new_parent = store_->FindInodeLocked(new_parent_ino);
  if (!new_parent) {
  ...
  if (store_->FindEntryLocked(new_parent_ino, new_name) != nullptr) {
    return Status::AlreadyExists("target entry already exists");
  }

  SwordFsInode *child = store_->UnlinkEntryLocked(old_parent_ino, old_name);
  if (!child) {
    return Status::Internal("unlink entry failed");
  }
  child->parent_ino = new_parent_ino;
  store_->LinkEntryLocked(new_parent_ino, new_name, child);
  return Status::OK();
}

Status MemMetaTxn::Unlink(InodeID parent_ino, std::string_view name,
                          nlink_t *post_nlink) {
  SwordFsInode *child = store_->FindEntryLocked(parent_ino, name);
  if (!child) {
  ...
  if (child->IsDir() && !store_->IsDirEmptyLocked(child->ino)) {
  ...
  store_->UnlinkEntryLocked(parent_ino, name);

  if (child->IsDir()) {
    // Directories cannot be hard-linked; always reclaim immediately.
    store_->DeleteInodeLocked(child->ino);
    if (post_nlink) {
...
Status MemMetaTxn::LinkExistingEntry(InodeID parent_ino,
  ...
  inode->attr.st_nlink++;
  store_->LinkEntryLocked(parent_ino, name, inode);
  return Status::OK();
}

Status MemMetaTxn::ListEntries(InodeID ino,
                               std::vector<SwordFsEntry> *entries) {
  SwordFsInode *dir = store_->FindInodeLocked(ino);
  if (dir == nullptr) {
