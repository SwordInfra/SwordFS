  ...
  auto worker = [&](int tid) {
    for (int i = 0; i < kPerThread; ++i) {
      ...
      Status status = store_->Transact([&](MemMetaTxn &txn) {
        return txn.AddEntry(kRoot, name, kRegFileType, kRegFilePermissions, nullptr);
      });
      if (status.ok()) {
...
TEST_F(MemMetaStoreConcurrencyTest, ConcurrentAddEntrySameName) {
  ...
  auto worker = [&]() {
    ...
    SwordFsInode child;
    Status status = store_->Transact([&](MemMetaTxn &txn) {
      return txn.AddEntry(kRoot, "race_target", kRegFileType, kRegFilePermissions, &child);
    });
    if (status.ok()) {
      success_count.fetch_add(1, std::memory_order_relaxed);
...
TEST_F(MemMetaStoreConcurrencyTest, ConcurrentMoveEntryAtomicity) {
  ...
  store_->Transact([&](MemMetaTxn &txn) {
    return txn.AddEntry(kRoot, "src", kDirType, kDirPermissions, &src_dir);
  });
  SwordFsInode dst_dir;
  store_->Transact([&](MemMetaTxn &txn) {
    return txn.AddEntry(kRoot, "dst", kDirType, kDirPermissions, &dst_dir);
  });
  SwordFsInode f;
  store_->Transact([&](MemMetaTxn &txn) {
    return txn.AddEntry(src_dir.ino, "target", kRegFileType, kRegFilePermissions, &f);
  });
...
TEST_F(MemMetaStoreConcurrencyTest, ConcurrentRemoveAndAdd) {
  ...
  for (int i = 0; i < kFiles; ++i) {
    SwordFsInode f;
    store_->Transact([&](MemMetaTxn &txn) {
      return txn.AddEntry(kRoot, "file_" + std::to_string(i), kRegFileType, kRegFilePermissions, &f);
    });
...
TEST_F(MemMetaStoreConcurrencyTest, ConcurrentMoveToSameTarget) {
  ...
  store_->Transact([&](MemMetaTxn &txn) {
    return txn.AddEntry(kRoot, "dst", kDirType, kDirPermissions, &dir_dst);
  });

  SwordFsInode f1, f2;
  store_->Transact([&](MemMetaTxn &txn) {
    return txn.AddEntry(dir_a.ino, "file", kRegFileType, kRegFilePermissions, &f1);
  });
  store_->Transact([&](MemMetaTxn &txn) {
    return txn.AddEntry(dir_b.ino, "file", kRegFileType, kRegFilePermissions, &f2);
  });
