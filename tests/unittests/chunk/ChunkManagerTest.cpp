...
// Helper: wrap a string in a folly::IOBuf.
static auto Buf(const std::string &s) {
  return *folly::IOBuf::copyBuffer(s.data(), s.size());
}

// Helper: run |fn| inside a folly fiber on a temporary EventBase.
...
TEST_F(ChunkManagerReadTest, FullChunk) {
  RunInTestFiber([&] {
    ...
    rw.Write(Buf(Repeat('A', kChunkSize)), 0);
...
TEST_F(ChunkManagerReadTest, PartialChunkAtEOF) {
  RunInTestFiber([&] {
    // Write a partial chunk (500 bytes into a 1024-byte chunk).
    swordfs::vfs::FileReadWriter rw(kFh, 42, kChunkSize);
    rw.Write(Buf(Repeat('A', 500)), 0);
    auto out = folly::IOBuf::create(kChunkSize);
    ASSERT_TRUE(rw.Read(kChunkSize, 0, out.get()).ok());
    // 500 bytes of 'A' + remaining zeros.
...
TEST_F(ChunkManagerReadTest, CrossChunkBoundary) {
  RunInTestFiber([&] {
    swordfs::vfs::FileReadWriter rw(kFh, 42, kChunkSize);
    rw.Write(Buf(Repeat('A', kChunkSize)), 0);
    rw.Write(Buf(Repeat('B', 500)), kChunkSize);
...
TEST_F(ChunkManagerReadTest, CrossChunkReadsIntoSecondChunk) {
  RunInTestFiber([&] {
    swordfs::vfs::FileReadWriter rw(kFh, 42, kChunkSize);
    rw.Write(Buf(Repeat('A', kChunkSize)), 0);
    rw.Write(Buf(Repeat('B', 800)), kChunkSize);
...
TEST_F(ChunkManagerReadTest, CrossChunkExhaustsSecondChunk) {
  RunInTestFiber([&] {
    swordfs::vfs::FileReadWriter rw(kFh, 42, kChunkSize);
    rw.Write(Buf(Repeat('A', kChunkSize)), 0);
    rw.Write(Buf(Repeat('B', 200)), kChunkSize);
    ...
    // 50 'A's from chunk 0, 200 'B's from chunk 1, 6 zeros.
...
TEST_F(ChunkManagerReadTest, CrossChunkSecondChunkMissing) {
  RunInTestFiber([&] {
    // Chunk 0 has data, chunk 1 is a hole — fill with zeros.
    swordfs::vfs::FileReadWriter rw(kFh, 42, kChunkSize);
    rw.Write(Buf(Repeat('A', kChunkSize)), 0);
    ...
    // 50 bytes of 'A' from chunk 0, then 150 zeros from hole.
...
TEST_F(ChunkManagerReadTest, SparseReadWithMultipleHoles) {
  RunInTestFiber([&] {
    // Chunk 0 has data, chunks 1+2 are holes, chunk 3 has data.
    swordfs::vfs::FileReadWriter rw(kFh, 42, kChunkSize);
    rw.Write(Buf(Repeat('A', kChunkSize)), 0);
    rw.Write(Buf(Repeat('B', kChunkSize)), kChunkSize * 3);

    auto out = folly::IOBuf::create(kChunkSize * 4);
    ASSERT_TRUE(rw.Read(kChunkSize * 4, 0, out.get()).ok());
    // Chunk 0: 'A'*256, Chunk 1+2: '\0'*512, Chunk 3: 'B'*256
    std::string expected =
        Repeat('A', kChunkSize) +
...
TEST_F(ChunkManagerReadTest, ReadPastEndOfBufferToStorage) {
  RunInTestFiber([&] {
    swordfs::vfs::FileReadWriter rw(kFh, 42, kChunkSize);
    rw.Write(Buf(Repeat('B', kChunkSize)), 0);
    rw.Write(Buf(Repeat('B', 300)), kChunkSize);
...
TEST_F(ChunkManagerReadTest, ReadBufferTakesPrecedenceOverStorage) {
  RunInTestFiber([&] {
    swordfs::vfs::FileReadWriter rw(kFh, 42, kChunkSize);
    rw.Write(Buf(Repeat('B', 200)), 0);

    auto out = folly::IOBuf::create(kChunkSize);
    ASSERT_TRUE(rw.Read(kChunkSize, 0, out.get()).ok());
    // 200 bytes of 'B', then zeros for the rest.
    std::string expected = Repeat('B', 200) +
                           std::string(kChunkSize - 200, '\0');
    EXPECT_EQ(out->length(), kChunkSize);
    EXPECT_EQ(std::string_view(reinterpret_cast<const char *>(out->data()),
                               out->length()),
              expected);
...
TEST_F(ChunkManagerFlushTest, FlushUploadsPartialChunk) {
  swordfs::vfs::FileReadWriter rw(kFh, kIno, kChunkSize);
  rw.Write(Buf(Repeat('Y', kChunkSize / 2)), 0);
...
TEST_F(ChunkManagerFlushTest, FlushUploadsFullChunks) {
  swordfs::vfs::FileReadWriter rw(kFh, kIno, kChunkSize);
  rw.Write(Buf(Repeat('X', kChunkSize * 2)), 0);
...
TEST_F(ChunkManagerFlushTest, ChunkIndexIncrementsCorrectly) {
  swordfs::vfs::FileReadWriter rw(kFh, kIno, kChunkSize);
  rw.Write(Buf(Repeat('X', kChunkSize * 3)), 0);
  rw.Flush();
